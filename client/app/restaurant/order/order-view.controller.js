  ...
  .controller('ViewOrderCtrl', ['$scope', '$location', '$state','$stateParams','$cookieStore','Auth','User','Order','Printer',
    ...
    var initPrintModel = function (order){
      ...
      if(order.coupon_id){
        ...
        str+='<div style="width:90px;float:left;font-size:20px;;text-align:right;font-weight:bold">'+order.coupon_name+'</div>';
        str+='<div style="clear:both"></div></div><div><div style="width:160px;float:left">优惠金额(CouponValue):</div>';
        str+='<div style="width:90px;float:left;font-size:20px;;text-align:right;font-weight:bold">'+order.coupon_value+'</div>';
        str+='<div style="clear:both"></div></div><div><div style="width:160px;float:left">最终实收(FinalTotal):</div>';
        str+='<div style="width:90px;float:left;font-size:20px;;text-align:right;font-weight:bold">'+order.coupon_after+'</div>';
      }
      str+='<div style="clear:both"></div></div></div>';
      str+='<hr style=" height:2px;border:none;border-top:2px dotted #000;" />';
