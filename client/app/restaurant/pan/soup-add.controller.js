...
angular.module('kuaishangcaiwebApp')
  .controller('AddSoupCtrl', ['$scope', '$location', '$state','$stateParams','$cookieStore','Auth', 'Attribute', 'Upload', 'Compress_ready', 'Pan_soup',
    ...
    self.pan_soup = {
		...
		attribute:[{
			...
			value:[{
				name:'',
				name_english:'',
				isDefault:''//是否为默认口味，若全部选项没有，默认为第一个，有多个则第一个true有效
			}]
		}],
		describe:'',
		describe_english:'',
    ...
    self.save = function (){
        ...
        // }
        Pan_soup.create({},self.pan_soup,function (data){
            if(data.code){
                init();
